...
public:
    ...
    //解析客户请求时，主状态机所处的状态
    enum CHECK_STAT {CHECK_STAT_REQUESTLINE=0, CHECK_STAT_HEADER, CHECK_STATE_CONTENT};
    //服务器处理HTTP请求的可能结果
...
private:
    ...
    HTTP_CODE parse_request_line(char* next);
    HTTP_CODE parse_headers(char* next);
    HTTP_CODE parse_content(char* next);
...
private:
    ...
    //我们将利用wrtieev来执行写操作，所以定义下面两个成员，其中m_iv_count标识被写内存快的数量
    struct iovec m_iv[2];
    int m_iv_count;
};
